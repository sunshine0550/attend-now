 ...
 * 며칠에 걸친 시도를 막지 못한다. 전화번호별로 실패 횟수를 세서 잠근다.
 *
...
export async function recordFailedLogin(phone: string) {
  await supabase.from('login_attempts').insert({ phone })
}

/** 로그인 성공 시 그 번호의 실패 기록을 지운다 */
export async function clearLoginAttempts(phone: string) {
